...
class MLPDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.future_steps = future_steps # ?意思都不知道
        ...
        self.loc = nn.Sequential(
            ...
            nn.LayerNorm(self.hidden_size),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_size, self.future_steps * 2)) # why future_step*2      
        # 什么意义？？ 为什么uncertain self.scale才有值！！！
        # uncertain: 多条可能的轨迹分布！！！ multi-modal uncetainty
        if uncertain:
            self.scale = nn.Sequential(
                nn.Linear(self.hidden_size, self.hidden_size),
                nn.LayerNorm(self.hidden_size),
                nn.ReLU(inplace=True),
                nn.Linear(self.hidden_size, self.future_steps * 2)) # self.scale=self.loc啊！！！whyb不这样写            
    ...
    def forward(self,
        ...
        out = self.aggr_embed(torch.cat((global_embed, 
                                         local_embed.expand(self.num_modes, *local_embed.shape)), dim=-1))
...
class GRUDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if uncertain:
            # 如果输出多条可能轨迹的分布 multi-modal模式 not uni-modal
            self.scale = nn.Sequential(
                nn.Linear(self.hidden_size, self.hidden_size),
                nn.LayerNorm(self.hidden_size),
                nn.ReLU(inplace=True),
    ...
    def forward(self,
        ...
        # 比mlp多了这个,为了设置 global_embed是input, local_embed是h_0.  看pytorch的自带的源码里的解释
        ...
        out, _ = self.gru(global_embed, local_embed) # gru输出out,hidden state
